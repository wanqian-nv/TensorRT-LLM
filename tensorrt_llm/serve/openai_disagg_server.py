...
class RawRequestResponseHooks(ResponseHooks):
    ...
    def on_resp_done(
            self, gen_server: str, request: UCompletionRequest,
            response: UCompletionResponse = None):
        # Stamped before the collect_perf_metrics gate: the Anthropic routes
        # read raw_request.state.ctx_request_id whether or not perf metrics
        # are being collected.
        self._record_disagg_request_ids(request)
        self.gen_server = gen_server
...
class OpenAIDisaggServer:
    def __init__(self,
        ...
        async def validation_exception_handler(request: Request, exc):
            ...
            # Anthropic clients need the error shaped their way. Placed after
            # the sampled logging so /v1/messages failures still get logged.
