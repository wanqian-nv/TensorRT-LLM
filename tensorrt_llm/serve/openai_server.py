...
class OpenAIServer(_VideoRoutesMixin):
    ...
    async def _prepare_chat_prompt(
            ...
            raw_request: Request
    ) -> tuple[dict, List[ConversationMessage], Optional[str]]:
        """Build the exact prompt inputs shared by generation and token counting.

        Returns the prompt, the parsed conversation, and the rendered prompt
        string when the chat template produced one -- callers need the latter
        to tell which thinking mode the template rendered.
        """
        ...
        return prompt, conversation, rendered_prompt
    ...
    async def _count_chat_prompt_tokens(self, request: ChatCompletionRequest,
        ...
        prompt, _, _ = await self._prepare_chat_prompt(request, raw_request)
    ...
    async def openai_chat(self, request: ChatCompletionRequest,
        ...
        try:
            ...
            prompt, conversation, rendered_prompt = await self._prepare_chat_prompt(
                request, raw_request)
