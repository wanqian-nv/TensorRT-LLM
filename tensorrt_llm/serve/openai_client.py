        ...
        for attempt in range(loop_max):
            ...
            try:
                ...
                async with self._session.post(
                    ...
                    # Collect perf metrics before the error check below: an
                    # error response carries no metrics headers, so this is a
                    # no-op there, but it keeps metrics for every response that
                    # does carry them.
                    if self._request_perf_metrics:
